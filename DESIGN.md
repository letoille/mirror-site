...
- **一句话定位**：《流放之路》1 & 2（POE1 · POE2）桌面悬浮窗助手的官方介绍 + 下载页。
...
| **Hero** | 主标题（流光"魔镜"）· 副标题（点明 POE1 & POE2）· 下载 CTA · 访问/下载计数 · 三服+语言+**手柄徽章** · 右侧模拟悬浮窗（纯 CSS，不放视频，作品牌锚点） |
| **功能区**（`#highlights`） | **10 张大卡**，左右交错排版（严格 normal/flip 交替），每张：标题 + 描述 + 要点 + 演示 |
...
### 10 大功能（对应 app 的 overlay）
截图识别查价 · 剧情引导 · **AI 市集搜索（即将上线）** · 物品查价 · 快捷正则 · 唯一物品搜索 · 通货兑换 · 计时器 · **奖励计数（POE1 & POE2）** · **收益统计（仅 POE1）**。
支持：国际服/国服/台服 · 英/简/繁 · 键盘 + 手柄 · **POE1 与 POE2 双游戏**。

> **双游戏定位**：站点已从纯 POE2 升级为 POE1 & POE2 并列（title/description/keywords/OG/hero/JSON-LD/FAQ 均已点明两代）。收益统计（仓库估价 + 打金收益/时薪/每图收益）**仅 POE1**（GGG 仓库接口只支持 POE1），卡片标题挂 `.poe1-chip`（蓝宝石色胶囊，区别于金色的"即将上线" `.soon-chip`）。奖励计数两代通用。改动词请保持这条准确。
...
- `reward` 竖向奖励清单 mock（地图 ×次数 + 掉落行 + 总 div，`.mk-reward`）；`wealth` 收益摘要 mock（追踪资产 + 时薪 + 资产行，`.mk-wealth`）。**这两个暂无真实视频**，靠 mock 撑着——录好丢 `demos/reward_<lang>.mp4` / `demos/wealth_<lang>.mp4` 即可（收益统计界面文字都是中文，可考虑用 `data-nolang` 只录一版）。
