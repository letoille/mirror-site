...
文本是给不存在的图配的，图片搜索拿不到。### 魔镜网站那三张

`/web.html` 的工具卡顶部也有图位，拍的是**网页**不是客户端：

| 槽位 | 拍什么 | 建议尺寸 |
|---|---|---|
| `web-market` | 市集搜索页：选定类别后收窄的词缀表单 + 结果列表 | 1280×800 |
| `web-character` | 角色页：载入角色后的面板，最好带上换装差值 | 1280×800 |
| `web-regex` | 正则生成器：勾选条件那一栏 + 生成出来的表达式 | 1280×800 |

⚠️ **浏览器截图别带地址栏和书签栏** —— 那是另一个人的浏览器，不是产品的一部分，
而且会把真实标签页标题泄露出去。
